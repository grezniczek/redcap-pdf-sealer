...
const POLICY_OID = '1.3.6.1.4.1.55555.3161.1';
...
$now = time();
$client = new Client(new Config('http://localhost.test/tsa', policyOid: POLICY_OID));
...
$noNonceClient = new Client(new Config('http://localhost.test/tsa', policyOid: POLICY_OID, nonceEnabled: false));
...
$defaultPolicyClient = new Client(new Config('http://localhost.test/tsa', nonceEnabled: false));
...
check(statusCode($service->respond($wrongPolicyRequest, $identity, $now), $asn1) === 2, 'Unknown policy was accepted');
